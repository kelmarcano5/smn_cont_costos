package domain.repgen.smn_configuracion.smn_rel_centro_costo_elemento;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rce_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_elemento_id_name");
			if (d != null)
			{
			name  = "b_element";
			label = "${lbl:b_element}";
		...
		d = rs.getString("smn_elemento_id");
			if (d != null)
			{
			name  = "b_element";
			label = "${lbl:b_element}";
		...
		d = rs.getString("rce_fecha_registro_fin");
			if (d != null)
			{
