package domain.repgen.smn_configuracion.smn_bases_distribucion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("bdi_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("bdi_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("bdi_forma_calculo");
			if (d != null)
			{
			name  = "b_forma_calculo";
			label = "${lbl:b_forma_calculo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("bdi_descripcion");
			if (d != null)
			{
			name  = "b_description";
