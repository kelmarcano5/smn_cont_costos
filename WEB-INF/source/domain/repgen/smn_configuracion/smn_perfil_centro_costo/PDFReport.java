package domain.repgen.smn_configuracion.smn_perfil_centro_costo;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_nivel_distribucion_id");
			if (d != null)
			{
			name  = "b_nivel_distribucion";
			label = "${lbl:b_nivel_distribucion}";
		...
		d = rs.getString("cdi_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_doc_base_distribucion_id_name");
			if (d != null)
			{
			name  = "b_documento_base_distribucion";
			label = "${lbl:b_documento_base_distribucion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_doc_base_distribucion_id");
			if (d != null)
			{
			name  = "b_documento_base_distribucion";
			label = "${lbl:b_documento_base_distribucion}";
		...
		d = rs.getString("cdi_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_nivel_distribucion_id_name");
			if (d != null)
			{
			name  = "b_nivel_distribucion";
			label = "${lbl:b_nivel_distribucion}";
		...
		d = rs.getString("smn_funcion_centro_id_name");
			if (d != null)
			{
			name  = "b_funcion_centro_costo";
			label = "${lbl:b_funcion_centro_costo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_funcion_centro_id");
			if (d != null)
			{
			name  = "b_funcion_centro_costo";
			label = "${lbl:b_funcion_centro_costo}";
